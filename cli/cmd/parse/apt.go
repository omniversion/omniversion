...
func parseAptOutput(input string) ([]models2.Dependency, error) {
	...
	result := make([]models2.Dependency, 0, len(matches))
	...
	for _, match := range matches {
		...
		if outdatedVersion == "" {
			...
			if isInstalled {
				...
				newDependency.Installed = []models2.InstalledDependency{{
					Version: version,
				}}
			} else {
